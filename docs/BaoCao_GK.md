...
Em thực hiện đề tài **Web Live Stream** trong khuôn khổ môn **Chuyên đề phát triển phần mềm** (bài giữa kỳ). Đây là một trang web cho phép một người **phát** hình ảnh và âm thanh từ camera và micro trên trình duyệt, và người khác **xem trực tiếp** bằng mã phòng hoặc link chia sẻ.
...
Báo cáo được trình bày theo thứ tự: giới thiệu sản phẩm → công nghệ và cách làm → hướng dẫn chạy thử và demo → kết quả, hạn chế, hướng phát triển → tài khoản quản trị.
...
| API bổ trợ | REST: kiểm tra phòng (`/api/room/:id/exists`), kiểm tra hoạt động (`/api/health` có `version`) |
...
### III.3. Sơ đồ kiến trúc và luồng WebRTC

**Cách xem sơ đồ:** các khối `mermaid` hiển thị trên GitHub hoặc VS Code (Markdown Preview Mermaid). Để chèn vào Word/PowerPoint, xuất PNG tại [mermaid.live](https://mermaid.live) (dán mã bên dưới).

#### III.3.1. Kiến trúc tổng quan (REST, WebSocket, P2P, STUN)

```mermaid
flowchart TB
  subgraph BH["Trình duyệt — Host"]
    HR["React HostLive"]
    HPC["WebRTC: getUserMedia + RTCPeerConnection"]
  end
  subgraph BV["Trình duyệt — Viewer"]
    VR["React WatchLive"]
    VPC["WebRTC: RTCPeerConnection"]
  end
  subgraph NODE["Máy chủ Node.js :3001"]
    REST["Express — REST"]
    WS["WebSocket /live-signal — liveRooms relay SDP/ICE"]
  end
  STUN[("STUN công khai\nstun.l.google.com:19302")]
  HR -->|"GET /api/room/.../exists, /api/health"| REST
  VR -->|"GET /api/room/.../exists"| REST
  HR <-->|"JSON signaling"| WS
  VR <-->|"JSON signaling"| WS
  HPC <-->|"RTP/SRTP trực tiếp (P2P)"| VPC
  HPC -.->|"ICE candidate"| STUN
  VPC -.->|"ICE candidate"| STUN
```

#### III.3.2. Trình tự signaling (một viewer vào phòng)

```mermaid
sequenceDiagram
  participant H as Host
  participant Sig as "Node — WebSocket /live-signal"
  participant V as Viewer
  H->>Sig: host-start
  Sig->>H: room-ready (roomId)
  V->>Sig: viewer-join (roomId)
  Sig->>V: joined (viewerId)
  Sig->>H: viewer-joined (viewerId)
  Note over H: addTrack, createOffer
  H->>Sig: offer + viewerId + sdp
  Sig->>V: offer + sdp
  Note over V: setRemoteDescription, createAnswer
  V->>Sig: answer + sdp
  Sig->>H: answer + viewerId + sdp
  par ICE song song
    H->>Sig: ice-host (viewerId, candidate)
    Sig->>V: ice (candidate)
    V->>Sig: ice-viewer (candidate)
    Sig->>H: ice (viewerId, candidate)
  end
  Note over H,V: RTP/SRTP — media trực tiếp P2P sau khi SDP/ICE xong
```

#### III.3.3. Bản tóm tắt in / PDF (khi Mermaid không được render)
...
### III.5. Quy mô mã nguồn (LOC) và ước lượng Story point
...
## IV. Cách chạy dự án, kiểm thử và hướng dẫn demo
...
### IV.2. Chạy backend (cửa sổ thứ nhất)
...
Đợi terminal báo server chạy (thường cổng **3001**).

### IV.3. Chạy frontend (cửa sổ thứ hai)
...
### IV.4. Kiểm thử tự động (nên quay trong video demo)
...
*(Trên Windows PowerShell, chạy từng lệnh trong đúng thư mục.)*

### IV.5. Demo luồng host → viewer
...
- Có **mã nguồn** trên GitHub và **báo cáo** (Word/PDF/PowerPoint) kèm **link video** theo yêu cầu giảng viên.
...
- Mở rộng **xác thực**, quản lý phòng, ghi log.
...
- **Sau khi đăng nhập:** vào **Quản lý hệ thống** (ví dụ đường dẫn `/system`).
...
- Trang **Đăng ký** trên website **chỉ** tạo tài khoản vai trò **người dùng thường** (user); backend **không** gán `sysmanager` hay `admin` qua API đăng ký công khai.
- **Quản lý hệ thống** và **Admin phòng live** **không** có chức năng “tự đăng ký” trên web; đây là yêu cầu an toàn để **không** lộ quyền cao qua form công khai.
- Hai tài khoản này **chỉ được khởi tạo phía máy chủ** khi chạy backend: đọc biến môi trường **`SEED_*`** trong file **`backend/.env`** (mẫu trong **`backend/.env.example`**) và ghi vào dữ liệu cục bộ nếu email chưa tồn tại. Sau đó người quản trị **đăng nhập** bằng email/mật khẩu đã cấu hình — **không** phải tạo mới qua trang Đăng ký.
