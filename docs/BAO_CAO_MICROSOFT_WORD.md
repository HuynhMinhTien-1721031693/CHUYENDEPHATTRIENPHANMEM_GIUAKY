...
3. Chạy backend (`npm run dev` hoặc `npm start`). Nếu **chưa có** user trùng email trong **`backend/data/app-data.json`**, hệ thống **tự tạo** hai tài khoản đúng vai trò.  
4. Mở **Đăng nhập** và đăng nhập bằng email/mật khẩu trong `.env` — đây là **đăng nhập**, không phải **đăng ký** tạo tài khoản quản trị.

*Nếu hai email đã tồn tại, hệ thống **không ghi đè** mật khẩu — dùng mật khẩu cũ hoặc chỉnh/xóa user trong `app-data.json` khi demo lại.*
